...
let colorSwitchers = [...document.querySelectorAll('.koshochka-widget-colors-list .item')];
let koshochkaWidgetPopup = document.getElementById("koshochkaWidgetPopup");

function getUserColor(e) {
	let userColorH = '0';
	let userColorS = '0%';
	console.log(colorSwitchers);
	let userColorsArr = [userColorH, userColorS];
	return userColorsArr;
}

colorSwitchers.forEach(el => {
...
releaseTheKraken.addEventListener("click", async () => {
	let [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
	chrome.scripting.executeScript({
		target: { tabId: tab.id },
		func: releaseTheKoshochka,
		args: [ getUserColor() ],
	});
});

function releaseTheKoshochka(userColorsArr) {
	let existingKoshochka = document.querySelectorAll('.koshochka-widget-wrapper');
	let selectedColorH = userColorsArr[0];
	let selectedColorS = userColorsArr[1];
	if (existingKoshochka.length == 0) {
		...
		style.innerHTML = `
			...
			.koshochka-widget-body {
				...
				background: radial-gradient(ellipse at top left, hsl(var(--base-color-h), var(--base-color-s), var(--base-color-l-light)), hsl(var(--base-color-h), var(--base-color-s), var(--base-color-l-dark)));
			}
