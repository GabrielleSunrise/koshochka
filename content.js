...
function changeKoshochkaColor(color) {
     let koshochkaElements = document.querySelectorAll('.koshochka-widget-shadow-wrapper');
     let colorArr = color.split('-');
     let selectedColorH = colorArr[1];
     let selectedColorS = colorArr[2] + '%';
     if (koshochkaElements.length > 0) {
          koshochkaElements.forEach(el => {
               el.style.setProperty('--base-koshochka-widget-color-h', selectedColorH);
               el.style.setProperty('--base-koshochka-widget-color-s', selectedColorS);
          });
     }
}
...
function releaseTheKoshochka(color) {
     let existingKoshochka = document.querySelectorAll('.koshochka-widget-shadow-wrapper');
     let colorArr = color.split('-');
     let selectedColorH = colorArr[1];
     let selectedColorS = colorArr[2] + '%';
     ...
     if (existingKoshochka.length == 0) {
          ...
          koshochka.classList.add('koshochka-widget-shadow-wrapper');
          const shadow = koshochka.attachShadow({ mode: 'open' });
          shadow.innerHTML = `
               <style>
                    :host {
               ...
               <div class="koshochka-widget-wrapper">
          ...
          koshochka.style.setProperty('--base-koshochka-widget-color-h', selectedColorH);
          koshochka.style.setProperty('--base-koshochka-widget-color-s', selectedColorS);

          const catObject = shadow.querySelector('.koshochka-widget-wrapper');
          function checkAndAddClasses() {
              if (!catObject.classList.contains('animated')) {
                  let timeout = Math.floor(Math.random() * (91)) + 10;
                    let timeout2 = timeout + 12000;
                    setTimeout(() => {
                        catObject.classList.add('animated');
                        const randomClass = Math.random() < 0.5 ? 'koshochka-widget-animated-right' : 'koshochka-widget-animated-left';
                        catObject.classList.add(randomClass);
                   }, timeout);
                    setTimeout(() => {
                       catObject.classList.remove('animated', 'koshochka-widget-animated-right', 'koshochka-widget-animated-left');
                   }, timeout2);
              }
          }
